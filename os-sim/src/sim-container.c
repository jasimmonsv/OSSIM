																   ...
																   SimDatabase   *database)
{
  ...
  gint           row2;
  gint           row3;
  gchar         *query = "SELECT policy.id, policy.priority, policy.descr, policy_time.begin_hour, policy_time.end_hour, policy_time.begin_day, policy_time.end_day FROM policy, policy_time WHERE policy.id = policy_time.policy_id AND policy.active != 0 order by policy.order;";
  gchar         *query2;
  gchar         *query3;
  gchar				 	*temp;
  ...
  if (dm)
  {
    ...
    // Host SRC Inet Address
		sim_policy_set_has_actions(policy,sim_container_policy_has_actions_in_db(database,sim_policy_get_id(policy)));
