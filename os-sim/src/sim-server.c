...
sim_server_load_role (SimServer	*server)
{
  ...
  SimConfig *config = server->_priv->config;
	...
	//load correlate role
	query = g_strdup_printf ("SELECT value FROM config WHERE conf = 'server_correlate'");
	...
	query = g_strdup_printf ("SELECT value FROM config WHERE conf = 'server_cross_correlate'");
	dm = sim_database_execute_single_command (ossim.dbossim, query);
	if (dm)
	{
 		value = (GdaValue *) gda_data_model_get_value_at (dm, 0, 0);
	...
	query = g_strdup_printf ("SELECT value FROM config WHERE conf = 'server_store'");
	dm = sim_database_execute_single_command (ossim.dbossim, query);
	if (dm)
	{
 		value = (GdaValue *) gda_data_model_get_value_at (dm, 0, 0);
	...
	query = g_strdup_printf ("SELECT value FROM config WHERE conf = 'server_qualify'");
	dm = sim_database_execute_single_command (ossim.dbossim, query);
	if (dm)
	{
 		value = (GdaValue *) gda_data_model_get_value_at (dm, 0, 0);
	...
	query = g_strdup_printf ("SELECT value FROM config WHERE conf = 'server_forward_alarm'");
	dm = sim_database_execute_single_command (ossim.dbossim, query);
	if (dm)
	{
 		value = (GdaValue *) gda_data_model_get_value_at (dm, 0, 0);
	...
	query = g_strdup_printf ("SELECT value FROM config WHERE conf = 'server_forward_event'");
	dm = sim_database_execute_single_command (ossim.dbossim, query);
	if (dm)
	{
 		value = (GdaValue *) gda_data_model_get_value_at (dm, 0, 0);
