      ...
      g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "insert_event_alarm: AA");
        ...
        g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "insert_event_alarm: query1:%s",query1);
	      g_free (query1);
				...
				event->backlog_id = sim_database_get_id(ossim.dbossim,BACKLOG_SEQ_TABLE);			
			}
														...
														SimPolicy		*policy)
{
  ...
  if (policy)
    ...
    if (!event->is_prioritized)  //Take care that if it's prioritized, it wont be changed
    {
  ...
  if ((event->plugin_id != SIM_PLUGIN_ID_DIRECTIVE)  && (!event->is_reliability_setted))
  {
	  gint aux;
    ...
    if (sim_directive_match_by_event (directive, event)) 
			...
			if (!G_NODE_IS_LEAF (node_root))	//if the node has some children...
	    {
	      ...
	      while (children)								
				...
				g_log(G_LOG_DOMAIN,G_LOG_LEVEL_DEBUG,"%s Name:%s Level of rule:%d", __FUNCTION__,sim_rule_get_name(r),sim_rule_get_level(r));		
				...
				host = sim_container_get_host_by_ia(ossim.container,event->dst_ia);
				nets = sim_container_get_nets_has_ia(ossim.container,event->dst_ia);
				if (host){
      			...
      			int best_mask = -1;
       			...
       			GList *list_inet = sim_net_get_inets(net);
				       while (list_inet){
          			SimInet *cmp = (SimInet *) list_inet->data;
          			if (sim_inet_has_inet(cmp, dst)){  //check if src belongs to cmp.
          				int mask = sim_inet_get_mask(cmp);
           				if (mask > best_mask) {
           				  best_mask = mask;
             				asset_dst = sim_net_get_asset (net);
           				}
          			}
          			list_inet = list_inet->next;
        			} // while inet
				...
				/* debug info*/
				double risk = ((double)(asset*reliability*priority))/25;
				g_log(G_LOG_DOMAIN,G_LOG_LEVEL_DEBUG,"%s First directive level: Asset:%d Reliability:%d Priority:%d Risk:%f",
																								...
																								gchar        *timestamp)
{
  ...
  query = g_strdup_printf ("INSERT IGNORE INTO ac_sensor_signature (sid,day, plugin_id, plugin_sid) VALUES (%u,DATE('%s'), %d, %d)", sid, timestamp, event->plugin_id, event->plugin_sid);
  sim_database_execute_no_query (db_snort, query);
  g_free (query);
  ...
  // AC_ALERTS queries
  query = g_strdup_printf ("INSERT INTO ac_alerts_signature (day,sig_cnt,first_timestamp,last_timestamp, plugin_id, plugin_sid) VALUES (DATE('%s'),1,'%s','%s', %d, %d) ON DUPLICATE KEY UPDATE sig_cnt=sig_cnt+1,last_timestamp='%s'", timestamp, timestamp, timestamp, event->plugin_id, event->plugin_sid, timestamp);
  sim_database_execute_no_query (db_snort, query);
  g_free (query);

  query = g_strdup_printf ("INSERT IGNORE INTO ac_alerts_sid (day,sid, plugin_id, plugin_sid) VALUES (DATE('%s'),%u, %d, %d)", timestamp, sid, event->plugin_id, event->plugin_sid);
  sim_database_execute_no_query (db_snort, query);
  g_free (query);

  query = g_strdup_printf ("INSERT IGNORE INTO ac_alerts_ipsrc (day,ip_src, plugin_id, plugin_sid) VALUES (DATE('%s'),%u, %d, %d)", timestamp, sim_inetaddr_ntohl (event->src_ia), event->plugin_id, event->plugin_sid);
  sim_database_execute_no_query (db_snort, query);
  g_free (query);

  query = g_strdup_printf ("INSERT IGNORE INTO ac_alerts_ipdst (day,ip_dst, plugin_id, plugin_sid) VALUES (DATE('%s'),%u, %d, %d)", timestamp, sim_inetaddr_ntohl (event->dst_ia), event->plugin_id, event->plugin_sid);
  sim_database_execute_no_query (db_snort, query);
  g_free (query);
...
*/
  ...
  query = g_strdup_printf ("INSERT IGNORE INTO ac_srcaddr_signature (ip_src,day, plugin_id, plugin_sid) VALUES (%u,DATE('%s'), %d, %d)", sim_inetaddr_ntohl (event->src_ia), timestamp, event->plugin_id, event->plugin_sid);
  sim_database_execute_no_query (db_snort, query);
  g_free (query);
  ...
  query = g_strdup_printf ("INSERT IGNORE INTO ac_dstaddr_signature (ip_dst,day, plugin_id, plugin_sid) VALUES (%u,DATE('%s'),%d, %d)", sim_inetaddr_ntohl (event->dst_ia), timestamp, event->plugin_id, event->plugin_sid);
  sim_database_execute_no_query (db_snort, query);
  g_free (query);
  ...
  query = g_strdup_printf ("INSERT IGNORE INTO ac_layer4_sport_signature (layer4_sport,ip_proto,day, plugin_id, plugin_sid) VALUES (%u,%u,DATE('%s'), %d, %d)", event->src_port, event->protocol, timestamp, event->plugin_id, event->plugin_sid);
  sim_database_execute_no_query (db_snort, query);
  g_free (query);
  ...
  query = g_strdup_printf ("INSERT IGNORE INTO ac_layer4_dport_signature (layer4_dport,ip_proto,day, plugin_id, plugin_sid) VALUES (%u,%u,DATE('%s'), %d, %d)", event->dst_port, event->protocol, timestamp, event->plugin_id, event->plugin_sid);
  sim_database_execute_no_query (db_snort, query);
  g_free (query);
