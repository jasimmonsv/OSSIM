  ...
  if (database->_priv->local_DB)
    {
      ...
      while (!GDA_IS_CONNECTION(database->_priv->conn)
          || !gda_connection_is_open(database->_priv->conn)) //if database connection is not open, try to open it.
        {
          g_message(
              "Error (1): DB Connection is closed. Trying to open it again....");
          database->_priv->conn = gda_client_open_connection_from_string(
              database->_priv->client, database->_priv->provider,
              database->_priv->dsn, GDA_CONNECTION_OPTIONS_DONT_SHARE);
      ...
      ret = gda_connection_execute_non_query(database->_priv->conn, command,
          NULL);

      if (ret < 0)
        {
          errors = gda_error_list_copy(gda_connection_get_errors(
              database->_priv->conn));
          for (i = 0; i < g_list_length(errors); i++)
            {
              error = (GdaError *) g_list_nth_data(errors, i);
...
sim_database_execute_single_command(SimDatabase *database, const gchar *buffer)
{
  ...
  while (!GDA_IS_CONNECTION(database->_priv->conn) || !gda_connection_is_open(
      database->_priv->conn))
    {
      g_message(
          "Error (2): DB Connection is closed. Trying to open it again....");
      database->_priv->conn = gda_client_open_connection_from_string(
          database->_priv->client, database->_priv->provider,
          database->_priv->dsn, GDA_CONNECTION_OPTIONS_DONT_SHARE);
      if (!database->_priv->conn)
  ...
  model = gda_connection_execute_single_command(database->_priv->conn, command,
      NULL);
...
sim_database_get_id(SimDatabase *database, gchar *table_name)
{
  ...
  g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
      "sim_database_get_id: id obtained: %d", id);
