...
sim_session_cmd_connect(SimSession *session, SimCommand *command)
{
  ...
  if (command->data.connect.hostname && strcmp(command->data.connect.hostname,
    ...
    {
      sim_session_set_hostname(session, g_strdup_printf(session->_priv->ip_str));
    }
    ...
    if (command->data.connect.tzone != 0) //update tzone for each sensor
      {
        gchar *query = g_strdup_printf(
            "UPDATE sensor SET tzone=\'%4.2f\' WHERE name=\'%s\'",
            command->data.connect.tzone, sim_sensor_get_name(sensor));
        g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
            "sim_session_cmd_connect: Setting tzone: %s", query);
      ...
      if (command->data.connect.version != NULL)
        {
          ...
          gchar
              *query =
                  g_strdup_printf(
                      "INSERT INTO sensor_properties (ip,version) VALUES (\"%s\",\"%s\") ON DUPLICATE KEY UPDATE version = \"%s\"",
                      ip, command->data.connect.version,
                      command->data.connect.version);
...
sim_session_cmd_event(SimSession *session, SimCommand *command)
{
  ...
  sim_container_set_sensor_event_number(ossim.container, SIM_EVENT_EVENT,
      sensor);
      ...
      if (for_this_server) //execute the command in this server.
        {
          ...
          case SIM_DB_ELEMENT_TYPE_PLUGINS:
            if (sim_session_is_sensor(session))
              return; //the sensor must able to load only the Policy
...
while ((!session->_priv->close) && (error = gnet_io_channel_readline(
    session->_priv->io, buffer, BUFFER_SIZE, &n)) == G_IO_ERROR_NONE && (n > 0))
  {
    g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
        "sim_session_read: Entering while. Session: %x", session);
    g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
        "sim_session_read: strlen(buffer)=%d; n=%d", strlen(buffer), n);
    g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "sim_session_read: Buffer: %s",
        buffer);
    ...
    if (error != G_IO_ERROR_NONE)
      {
        g_message("Received error, closing socket: %d: %s", error, g_strerror(
            error));
        return FALSE;
      }
    ...
    if (strlen(buffer) != n)
      {
        g_message(
            "Received error. Inconsistent data entry, closing socket. Received:%d Buffer lenght: %d: %d: %s",
            n, strlen(buffer), error, g_strerror(error));
        return FALSE;
      }
    /*
    ...
    if (strlen(buffer) <= 2)
      {
        g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
            "sim_session_read: Buffer <= 2 bytes");
        memset(buffer, 0, BUFFER_SIZE);
        continue;
        //			return FALSE;
      }
    ...
    if (!cmd)
      {
        g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
            "sim_session_read: error command null");
        continue; //we don't break the connection if the event is strange, we just reject the event
        //return FALSE;
      }

    g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
        "sim_session_read: Command from buffer type:%d ; id=%d", cmd->type,
        cmd->id);

    if (cmd->type == SIM_COMMAND_TYPE_NONE)
      {
        g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
            "sim_session_read: error command type none");
        g_object_unref(cmd);
        return FALSE;
      }
    ...
    if (sim_session_get_is_initial(session)) //is this the session started in sim_container_new();?
      {
        g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
            "sim_session_read: This is a initial session load");
        if (cmd->type == SIM_COMMAND_TYPE_DATABASE_ANSWER)
          ...
          {
            g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
                "sim_session_read: Mutex lock in OK");
            ...
            g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
                "sim_session_read: Mutex unlock in OK");
          }
        else
          {
            g_message(
                "Error: someone has tried to connect to the server when it still hasn't loaded everything needed");
    ...
    case SIM_COMMAND_TYPE_EVENT: //from sensor / server children
      ...
      if ((!config->server.role->correlate)
          ...
          && (!config->server.role->store) && (!config->server.role->qualify)
    ...
    default:
      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "sim_session_read: error command unknown type");
      res = sim_command_new_from_type(SIM_COMMAND_TYPE_ERROR);
      res->id = cmd->id;
...
g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
    "sim_session_read: exiting function in session: %x", session);
