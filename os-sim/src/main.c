...
sim_free_ossim()
{
  ...
  if (ossim.log.filename)
    g_free(ossim.log.filename);
  if (ossim.config)
    g_object_unref(ossim.config);
  if (ossim.container)
    g_object_unref(ossim.container);
  if (ossim.organizer)
    g_object_unref(ossim.organizer);
  if (ossim.scheduler)
    g_object_unref(ossim.scheduler);
  if (ossim.server)
    g_object_unref(ossim.server);
  if (ossim.dbossim)
    g_object_unref(ossim.dbossim);
  if (ossim.dbosvdb)
    g_object_unref(ossim.dbosvdb);
  if (ossim.dbsnort)
    g_object_unref(ossim.dbsnort);
...
{
  int fd_pid;
  if ((fd_pid = open(OS_SIM_RUN_DIR, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR
      | S_IWUSR)) < 0)
  ...
  else
    {
      ...
      if (lockf(fd_pid, F_TLOCK, 0) < 0)
        g_message(
            "Can't lock pid file; may be that another server process is running?");
      else
        {
          sprintf(pid_str, "%d\n", getpid());
...
main(int argc, char *argv[])
{
  ...
  g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
      "Starting OSSIM server debug with process id: %d", getpid());
  ...
  if (!ds)
    {
      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "Failed to load OSSIM DB XML Config");
  ...
  if (!ds)
    {
      ...
      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG,
          "Failed to load SNORT DB XML Config");
    }
  ...
  if (!ds)
    {
      g_print(
          "Error / Warning: OSVDB DB XML Config. If you want to use OSVDB please insert data load into config.xml. If you think that you don't need to use it, or you're running a server without DB in multiserver mode, just ignore this error.\n");
      g_log(
          G_LOG_DOMAIN,
          G_LOG_LEVEL_DEBUG,
          "Error / Warning: OSVDB DB XML Config. If you want to use OSVDB please insert data load into config.xml. If you think that you don't need to use it, or you're running a server without DB in multiserver mode, just ignore this error.");
