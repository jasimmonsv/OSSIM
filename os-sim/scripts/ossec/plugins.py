...
for p in plugins.keys():
	print 'DELETE FROM plugin WHERE id = "%s";' % plugins[p]
	print 'DELETE FROM plugin_sid where plugin_id = "%s";' % plugins[p]
	print 'INSERT INTO plugin(id, type, name, description) VALUES(%s, 1, "ossec-%s", "%s");' % (plugins[p], p, p)
