...
def readSQLFile(sql_file, dic_plugin_id_gname, sql_groupnames_list):
    ...
    reg_str = "INSERT INTO plugin\(id, type, name, description\) VALUES\((?P<plugin_id>\d+), \d+, \"ossec-(?P<gname>.*)\", \"(.*)\"\);"
...
#    config.add_section("translation")
    ...
    for pid, gname in dic_plugin_id_gname.items():
        ...
        s3 = 'INSERT INTO plugin(id, type, name, description) VALUES(%s, 1, "ossec-%s", "%s");\n' % (pid, gname, gname)
        print s1
        print s2
        print s3
    ...
    for rule_id, pinfo in pluginInfo_dic.items():
        ...
        s1 = 'INSERT INTO plugin_sid(plugin_id, sid, category_id, class_id, reliability, priority, name) VALUES(%s, %s, NULL, NULL, 1,1, "ossec: %s");\n' % (pid, sid, description.replace('"','\\"'))
        print s1
        sql_statements.append(s1)
    print "SQLS -------------------------------END"
