...
* - result_dump_mem()
* Classes list:
* - php4DOMAttr extends php4DOMNode
* - php4DOMDocument extends php4DOMNode
* - php4DOMElement extends php4DOMNode
* - php4DOMNode
* - php4DomProcessingInstruction extends php4DOMNode
* - php4DOMText extends php4DOMNode
* - php4DOMNodelist
* - php4DOMXPath
* - php4DomXsltStylesheet
*/
...
class php4DOMDocument extends php4DOMNode {
    ...
    function get_elements_by_tagname($name) {
        ...
        $nodeSet = array();
        $i = 0;
        if (isset($myDOMNodeList)) while ($node = $myDOMNodeList->item($i++)) $nodeSet[] = new php4DOMElement($node, $this);
        return $nodeSet;
    }
...
class php4DOMElement extends php4DOMNode {
    ...
    function get_elements_by_tagname($name) {
        ...
        $nodeSet = array();
        $i = 0;
        if (isset($myDOMNodeList)) while ($node = $myDOMNodeList->item($i++)) $nodeSet[] = new php4DOMElement($node, $this->myOwnerDocument);
        return $nodeSet;
    }
    ...
    function set_name($name) {
        ...
        $i = 0;
        if (isset($myDOMNodeList)) while ($node = $myDOMNodeList->item($i++)) if ($node->namespaceURI == '') $newNode->setAttribute($node->name, $node->value);
        else $newNode->setAttributeNS($node->namespaceURI, $node->nodeName, $node->value);
        $myDOMNodeList = $this->myDOMNode->childNodes;
        if (isset($myDOMNodeList)) while ($node = $myDOMNodeList->item(0)) $newNode->appendChild($node);
...
class php4DOMNode {
    ...
    function __get($name) {
        switch ($name) {
            ...
            case 'tagname':
                return $this->myDOMNode->tagName; //DomElement
                
            case 'content':
                return $this->myDOMNode->textContent;
            case 'name':
                return $this->myDOMNode->name; //DomAttribute
    ...
    function attributes() {
        ...
        $nodeSet = array();
        $i = 0;
        if (isset($myDOMNodeList)) while ($node = $myDOMNodeList->item($i++)) $nodeSet[] = new php4DOMAttr($node, $this->myOwnerDocument);
        return $nodeSet;
    }
    function child_nodes() {
        ...
        $nodeSet = array();
        $i = 0;
        if (isset($myDOMNodeList)) while ($node = $myDOMNodeList->item($i++)) $nodeSet[] = self::_newDOMElement($node, $this->myOwnerDocument);
        return $nodeSet;
    }
    function children() {
    ...
    function set_namespace($uri, $prefix = null) { //Contributions by Daniel Walker 2006-09-08
        ...
        if ($nsprefix == null) {
            ...
            if ($this->myDOMNode->nodeType === XML_ATTRIBUTE_NODE) {
                if (($prefix != null) && $this->myDOMNode->ownerElement->hasAttributeNS('http://www.w3.org/2000/xmlns/', $nsprefix) && ($this->myDOMNode->ownerElement->getAttributeNS('http://www.w3.org/2000/xmlns/', $nsprefix) != $uri)) { //Remove namespace
...
class php4DOMXPath {
    ...
    function xpath_eval($eval_str, $contextnode = null) {
        if (method_exists($this->myDOMXPath, 'evaluate')) return isset($contextnode) ? new php4DOMNodelist($this->myDOMXPath->evaluate($eval_str, $contextnode->myDOMNode) , $this->myOwnerDocument) : new php4DOMNodelist($this->myDOMXPath->evaluate($eval_str) , $this->myOwnerDocument);
