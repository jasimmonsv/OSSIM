	...
	<td valign="top">
		...
		if (count($database_servers)>0 && Session::menu_perms("MenuConfiguration", "PolicyServers") && preg_match("/pro|demo/i",$version)) { 
			...
			foreach ($database_servers as $db) {
				$svar = base64_encode($db->get_ip().":".$db->get_port().":".$db->get_user().":".$db->get_pass().":".$db->get_name().":end"); // 'end' tag to solve PHPIDS exception
			...
			echo "<td><a href='".preg_replace("/\&server\=[^\&]+/","",$actual_url)."server=local'>".($ss=="local" ? "<b>"._("Local")."</b>" : _("local"))."</a></td></tr>";
			echo "</table></td></tr>";
		}
		?>
		</table>
