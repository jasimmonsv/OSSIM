...
function CreateBASEAG($db) {
    ...
    if (!$tblBaseAG_present) {
        if ($db->DB_type == "mysql") {
            ...
            if ($db->baseErrorMessage() != "") ErrorMessage("Unable to CREATE table 'acid_ag': " . $db->baseErrorMessage());
            else ErrorMessage("Successfully created 'acid_ag'");
            $sql = 'CREATE SEQUENCE snort.seq_acid_ag_id START WITH 1 INCREMENT BY 1';
            $db->baseExecute($sql, -1, -1, false);
            if ($db->baseErrorMessage() != "") ErrorMessage("Unable to CREATE sequence 'snort.seq_acid_ag_id': " . $db->baseErrorMessage());
            else ErrorMessage("Successfully created sequence 'snort.seq_acid_ag_id'");
            $sql = 'CREATE or replace TRIGGER tr_acid_ag_id
                   ...
                   END;/';
            $db->baseExecute($sql, -1, -1, false);
            if ($db->baseErrorMessage() != "") ErrorMessage("Unable to CREATE trigger 'tr_acid_ag_id': " . $db->baseErrorMessage());
            else ErrorMessage("Successfully created trigger 'tr_acid_ag_id'");
        }
    ...
    if (!$tblBaseAGAlert_present) {
        ...
        $db->baseExecute($sql, -1, -1, false);
        if ($db->baseErrorMessage() != "") ErrorMessage("Unable to CREATE table 'acid_ag_alert': " . $db->baseErrorMessage());
        else ErrorMessage("Successfully created 'acid_ag_alert'");
        $tblBaseAGAlert_present = $db->baseTableExists("acid_ag_alert");
    }
    if (!$tblBaseIPCache_present) {
        ...
        $db->baseExecute($sql, -1, -1, false);
        if ($db->baseErrorMessage() != "") ErrorMessage("Unable to CREATE table 'acid_ip_cache': " . $db->baseErrorMessage());
        else ErrorMessage("Successfully created 'acid_ip_cache'");
        $tblBaseIPCache_present = $db->baseTableExists("acid_ip_cache");
    }
    if (!$tblBaseEvent_present) {
        if ($db->DB_type == "mysql") {
            if ($db->baseGetDBversion() < 100) $sig_ddl = "signature      VARCHAR(255) NOT NULL,";
            else $sig_ddl = "signature      INT UNSIGNED NOT NULL,";
            $sql = 'CREATE TABLE acid_event ( sid                 INT UNSIGNED NOT NULL,
                                                cid                 INT UNSIGNED NOT NULL,' . $sig_ddl . '     
            ...
            if ($db->baseGetDBversion() < 100) $sig_ddl = "signature      TEXT NOT NULL,";
            else $sig_ddl = "signature      INT8 NOT NULL,";
            $sql = 'CREATE TABLE acid_event ( sid                 INT8 NOT NULL,
                                                cid                 INT8 NOT NULL,' . $sig_ddl . '
                                                sig_name            TEXT,
                                                sig_class_id        INT8,
                                                sig_priority        INT8,
            ...
            /* Otherwise it will unpredictably change the nullability. */
            if ($db->baseGetDBversion() < 100) $sig_ddl = "signature      TEXT NOT NULL,";
            else $sig_ddl = "signature      NUMERIC(10,0) NOT NULL,";
            $sql = 'CREATE TABLE acid_event ( sid                 NUMERIC(10,0) NOT NULL,
                                                cid                 NUMERIC(10,0) NOT NULL,' . $sig_ddl . '
        ...
        $db->baseExecute($sql, -1, -1, false);
        if ($db->baseErrorMessage() != "") ErrorMessage("Unable to CREATE table 'acid_event': " . $db->baseErrorMessage());
        else ErrorMessage("Successfully created 'acid_event'");
        $tblBaseEvent_present = $db->baseTableExists("acid_event");
    }
    if ($db->DB_type == "mssql") {
        ...
        $sql = 'CREATE INDEX acid_ag_ag_id_idx ON acid_ag (ag_id)
                   ...
                   CREATE INDEX acid_event_layer4_dport ON acid_event (layer4_dport)';
        $db->baseExecute($sql, -1, -1, false);
        if ($db->baseErrorMessage() != "") ErrorMessage("Unable to CREATE MSSQL BASE table indexes : " . $db->baseErrorMessage());
        else ErrorMessage("Successfully created MSSQL BASE table indexes");
    }
    ...
    if (!$tblBaseRoles_present) {
        ...
        $db->baseExecute($sql, -1, -1, false);
        if ($db->baseErrorMessage() != "") ErrorMessage("Unable to CREATE table 'base_roles': " . $db->baseErrorMessage());
        else ErrorMessage("Successfully created 'base_roles'");
        ...
        $db->baseExecute($sql, -1, -1, false);
        if ($db->baseErrorMessage() != "") ErrorMessage("Unable to INSERT roles: " . $db->baseErrorMessage());
        else ErrorMessage("Successfully INSERTED Admin role");
        // Authenticated User
        $sql = "INSERT INTO base_roles (role_id, role_name, role_desc) VALUES (10, 'user', 'Authenticated User');";
        $db->baseExecute($sql, -1, -1, false);
        if ($db->baseErrorMessage() != "") ErrorMessage("Unable to INSERT roles: " . $db->baseErrorMessage());
        else ErrorMessage("Successfully INSERTED Authenticated User role");
        // Anonymous User
        $sql = "INSERT INTO base_roles (role_id, role_name, role_desc) VALUES (10000, 'anonymous', 'Anonymous User');";
        $db->baseExecute($sql, -1, -1, false);
        if ($db->baseErrorMessage() != "") ErrorMessage("Unable to INSERT roles: " . $db->baseErrorMessage());
        else ErrorMessage("Successfully INSERTED Anonymous User role");
        // Alert Group Editor
        $sql = "INSERT INTO base_roles (role_id, role_name, role_desc) VALUES (50, 'ag_editor', 'Alert Group Editor');";
        $db->baseExecute($sql, -1, -1, false);
        if ($db->baseErrorMessage() != "") ErrorMessage("Unable to INSERT roles: " . $db->baseErrorMessage());
        else ErrorMessage("Successfully INSERTED Alert Group Editor role");
        $tblBaseRoles_present = $db->baseTableExists("base_roles");
    }
    if (!$tblBaseUsers_present) {
        ...
        $db->baseExecute($sql, -1, -1, false);
        if ($db->baseErrorMessage() != "") ErrorMessage("Unable to CREATE table 'base_users': " . $db->baseErrorMessage());
        else ErrorMessage("Successfully created 'base_users'");
        $tblBaseUsers_present = $db->baseTableExists("base_users");
    }
    if ($tblBaseAG_present && $tblBaseAGAlert_present && $tblBaseIPCache_present && $tblBaseEvent_present && $tblBaseRoles_present && $tblBaseUsers_present) return 1;
    else return 0;
}
?>
