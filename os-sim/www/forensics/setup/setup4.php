...
if ($result == 1) {
    ...
    if ($_SESSION['useuserauth'] == 1) {
        ...
        $db->baseExecute($sql);
        if ($db->baseErrorMessage() != "") ErrorMessage("Unable to CREATE User: " . $db->baseErrorMessage());
        else ErrorMessage("Successfully created user.");
        if ($userdelcnt[0] > 0) ErrorMessage("$userdelcnt[0] user(s) matching '$user' was(were) deleted.");
    }
} else echo '<INPUT TYPE="submit" NAME="submit" VALUE="Create BASE AG">';
echo '
  </TABLE>';
if ($result == 1) echo '<P>
