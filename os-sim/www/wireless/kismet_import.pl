...
sub parse_xml {
    ...
    foreach my $net (@items) {
        ...
        if ($aps_macs{$sensor}{$mac}{$ssid}) {
            $sql = qq{ UPDATE wireless_aps SET channel = '$channel', cloaked = '$cloaked', encryption = '$encryption', llc = GREATEST(llc, $llc), data = GREATEST(data, $data), carrier = '$carrier', lasttime = '$lasttime', datasize = GREATEST(datasize, $datasize) WHERE mac = '$mac' and ssid = '$ssid1' and sensor = '$sensor' };
            $sth_selm=$dbh->prepare( $sql );
            $sth_selm->execute;
            $sth_selm->finish;
            ...
            $sql_update = qq{ INSERT IGNORE INTO wireless_networks VALUES ('$ssid1','$sensor',0,0,'','','','','','Un-Trusted','','') };
            $sth_update = $dbh->prepare( $sql_update );
            $sth_update->execute;
            # wireless_aps
            $sql_update = qq{ INSERT IGNORE INTO wireless_aps VALUES ('$mac','$ssid1','$sensor','$nettype','$info','$channel','$cloaked','$encryption','$decrypted','$maxrate','$maxseenrate','$beacon','$llc','$data','$crypt','$weak','$dupeiv','$total','$carrier','$encoding','$firsttime','$lasttime','$bestquality','$bestsignal','$bestnoise','$gpsminlat','$gpsminlon','$gpsminalt','$gpsminspd','$gpsmaxlat','$gpsmaxlon','$gpsmaxalt','$gpsmaxspd','$gpsbestlat','$gpsbestlon','$gpsbestalt','$datasize','$iptype','$ip','') };
            $sth_update = $dbh->prepare( $sql_update );
            $sth_update->execute;
            $sth_update->finish;
        ...
        if (ref($net->{'wireless-client'}) eq 'ARRAY') {
            foreach $cl (@{$net->{'wireless-client'}}) {
                ...
                if ($clients_macs{$sensor}{$client_mac}{$ssid}) {
                    ...
                    # wireless_clients
                    $sql_update = qq{ INSERT IGNORE INTO wireless_clients VALUES ('$client_mac','$mac','$ssid1','$sensor','$plugin_sid','$channel','$encryption','$maxrate','$maxseenrate','$llc','$data','$crypt','$weak','$dupeiv','$total','$ctype','$encoding','$firsttime','$lasttime','$gpsminlat','$gpsminlon','$gpsminalt','$gpsminspd','$gpsmaxlat','$gpsmaxlon','$gpsmaxalt','$gpsmaxspd','$datasize','$iptype','$ip','') };
                    $sth_update = $dbh->prepare( $sql_update );
                    $sth_update->execute;
                    $sth_update->finish;
            ...
            $sql = qq{ select sid from snort.sensor where hostname like '$sensor-kismet'};
            $sth_sel=$dbh->prepare( $sql );
            $sth_sel->execute;
            if ( my ( $sid )=$sth_sel->fetchrow_array ) {
                $sql = "select distinct o.plugin_sid,e.userdata2,e.userdata3,e.userdata4 from snort.extra_data e, snort.acid_event o where o.sid=e.sid and o.cid=e.cid and o.plugin_id=1596 and o.plugin_sid not in (1,2) and o.sid=$sid and e.userdata2='$mac'";
                $sth_sel=$dbh->prepare( $sql );
                $sth_sel->execute;
                while ( my ( $plugin_sid,$mac_ssid,$client_mac,$ip )=$sth_sel->fetchrow_array ) {
                    ...
                    $client_mac=$ip_mac{$ip} if ($ip ne "" && $client_mac eq "");
                    $ipc = ($ip ne "") ? ",ip='$ip'" : "";
                    $sql_update = qq{ INSERT INTO wireless_clients VALUES ('$client_mac','$mac','$ssid1','$sensor','$plugin_sid','','','','','','','','','','','','','','','','','','','','','','','','','$ip','') ON DUPLICATE KEY UPDATE plugin_sid='$plugin_sid' $ipc };
                    $sth_update = $dbh->prepare( $sql_update );
                    $sth_update->execute;
...
sub gen_stats {
    ...
    $sql = qq{ select ssid from wireless_networks where sensor='$sensor' };
    $sth_sel=$dbh->prepare( $sql );
    $sth_sel->execute;
    while ( my ($ssid)=$sth_sel->fetchrow_array ) {
        ...
        # aps
        @addrs = ();
        $sql = qq{ select distinct mac from wireless_aps where ssid='$qssid' and sensor='$sensor' and nettype='infrastructure' };
        $aps=$dbh->prepare( $sql );
        $aps->execute;
        ...
        # clients
        $num_clients = 0;
        $sql = qq{ select count(distinct client_mac) from wireless_clients where ssid='$qssid' and sensor='$sensor' };
        $cls=$dbh->prepare( $sql );
        $cls->execute;
        ...
        # encryption
        %encr = ();
        $sql = qq{ select distinct encryption from wireless_aps where ssid='$qssid' and sensor='$sensor' };
        $enc=$dbh->prepare( $sql );
        $enc->execute;
        ...
        # cloaked
        %clk = ();
        $sql = qq{ select distinct cloaked from wireless_aps where ssid='$qssid' and sensor='$sensor' };
        $clo=$dbh->prepare( $sql );
        $clo->execute;
        ...
        # firsttime - lasttime
        $first  = $last = "";
        $sql = qq{ select min(firsttime),max(lasttime) from wireless_aps where ssid='$qssid' and sensor='$sensor' };
        $tt=$dbh->prepare( $sql );
        $tt->execute;
        ...
        $sql = qq{ update wireless_networks set aps=$num_aps,clients=$num_clients,encryption='$encryption',cloaked='$cloaked',firsttime='$first',lasttime='$last',macs='$macs' where ssid='$qssid' and sensor='$sensor' };
        $upt=$dbh->prepare( $sql );
        $upt->execute;
        $upt->finish;
    }
...
sub insert_sensor {
    ...
    my $sid = 0;
    my $sql = qq{ select sid from snort.sensor where hostname = '$sensor-Kismet'};
    ...
    if ($sid == 0) {
        ...
        my $sql_update = qq{ insert into snort.sensor (hostname,interface,encoding,last_cid) values ('$sensor-Kismet','eth0',2,0) on duplicate key update encoding=2 };
    ...
    if ($sname eq ""){
        # insert to ossim.wireless_sensors
        print "Inserting ossim.wireless_sensors Sensor_$sensor" if ($debug);
...
sub get_ips_macs {
    ...
    my $sql = qq{ select inet_ntoa(ip) as ip,mac from host_mac };
    my $sth_selm=$dbh->prepare( $sql );
    $sth_selm->execute;
...
sub get_aps_macs_and_clients {
    ...
    $sql = qq{ select distinct mac,ssid,sensor from wireless_aps };
    my $sth_selm=$dbh->prepare( $sql );
    $sth_selm->execute;
    ...
    $sql = qq{ select distinct client_mac,sensor,ssid from wireless_clients };
    my $sth_selm=$dbh->prepare( $sql );
    $sth_selm->execute;
