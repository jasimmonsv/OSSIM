...
<!--  <link rel="StyleSheet" href="dtree.css" type="text/css" />-->
  ...
  function toggle_group (group_id,name,ip_src,ip_dst,time) {
	document.getElementById(group_id).innerHTML = "<img src='../pixmaps/loading.gif'>";
	$.ajax({
		type: "GET",
		url: "alarm_group_response.php?name="+name+"&ip_src="+ip_src+"&ip_dst="+ip_dst+"&timestamp="+time+"&hide_closed=<?=$hide_closed?>",
		data: "",
  ...
  function opencloseAll () {
	...
	toggle_group('<?=$group['group_id']?>','<?=base64_encode($group['name'])?>','<?=$group['ip_src']?>','<?=$group['ip_dst']?>','<?=$group['date']?>');
	<? } ?>
	open = true;
	document.getElementById('expandcollapse').src='../pixmaps/minus.png';
	} else {
	<? foreach ($alarm_group as $group) { ?>
	untoggle_group('<?=$group['group_id']?>','<?=base64_encode($group['name'])?>','<?=$group['ip_src']?>','<?=$group['ip_dst']?>','<?=$group['date']?>');
	<? } ?>
	open = false;
...
<? foreach($alarm_group as $group) { 
	...
	<tr>
		...
		<td class="nobborder" id="plus<?=$group['group_id']?>"><a href="javascript:toggle_group('<?=$group['group_id']?>','<?=base64_encode($group['name'])?>','<?=$group['ip_src']?>','<?=$group['ip_dst']?>','<?=$group['date']?>');"><strong><img src='../pixmaps/plus-small.png' border=0></strong></a></td>
