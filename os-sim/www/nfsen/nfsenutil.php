...
function nfsend_connect ( ) {
	...
	if ( isset($_SESSION['nfsend']) ) {
		...
		if (!$nfsend) {
			...
			SetMessage('error', _("nfsend connect() error").": $errstr");
		...
		if ( ! $hello_string ) {
			...
			socket_close($nfsend);
			SetMessage('error', _("nfsend read() error").": $errstr");
			unset($_SESSION['nfsend']);
			return FALSE;
		}
...
// print "<h3>HELLO Status: $status Extra: $extra</h3>";
		if ( $status != 220 ) {
			SetMessage('error', _("nfsend connect error").": $extra");
			socket_close($nfsend);
			unset($_SESSION['nfsend']);
			return FALSE;
		}
...
function Plugin_Error( $plugin_id ) {
	...
	print "<h3>"._("Plugin Error! Can't run plugin")."</h3>\n";
...
function GetProfiles () {
	...
	if ( is_array($out_list) ) {
		...
		$profiles[] = _("New Profile ...");
	} else {
		$profiles = array();
		$profiles[] = "&lt;"._("No profiles available")."&gt;";
	}
...
function ReadProfile ($profileswitch) {
	...
	foreach ( $profileinfo['channel'] as $channel ) {
		...
		$has_perm = 0;
		foreach ($sensors_list as $s) if ($name == $s->get_name()) $has_perm = 1;
...
function ShowMessages () {
	...
	if ( array_key_exists('error', $_SESSION ) ) {
		foreach ( $_SESSION['error'] as $msg ) {
			print "<h3 class='errstring'>"._("ERROR").": $msg!</h3>\n";
		}
	...
	if ( array_key_exists('warning', $_SESSION ) ) {
		foreach ( $_SESSION['warning'] as $msg ) {
			print "<h3 class='warnstring'>"._("WARNING").": $msg!</h3>\n";
		}
	...
	if ( array_key_exists('alert', $_SESSION ) ) {
		foreach ( $_SESSION['alert'] as $msg ) {
			print "<h3 class='alertstring'>"._("ALERT").": $msg!</h3>\n";
		}
...
function SetMessage ($type, $message) {
	...
	if ( $type != 'info' && $type != 'alert' && $type != 'warning' && $type != 'error' ) {
		$type = 'error';
		$message = _('Internal error, setting message');
	}
