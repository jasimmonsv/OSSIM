...
function alert_name_check(&$new_alert, $opts ) {
	...
	$new_alert = preg_replace("/\s+$/", '', $new_alert);
	if ( $new_alert == '' ) {
		SetMessage('error', _("Empty alert name"));
		return 1;
	}
	...
	foreach ( $_SESSION['alertlist'] as $alert ) {
		if ( $new_alert == $alert ) {
			if ( $opts['must_exists'] ) {
				...
				SetMessage('error', _("Alert")." '$alert' "._("already exists"));
				return 1;
			}
		}
	}
...
function channellist_validate (&$channels, $opts) {
	...
	foreach ( $channels as $channel ) {
		if ( !array_key_exists($channel, $_SESSION['profileinfo']['channel'] ) ) {
			SetMessage('error', _("Channel")." '$channel' "._("does not exist in profile"));
			$channels = '';
			return 1;
		}
	}
...
<tbody>
	<tr class="ALERTTABLE">
		<th class="ALERTTABLE" colspan="2"><?=_("Filter applied to 'live' profile")?>:</th>
	</tr>
	<tr>
	<td style="width:15%">
...
<tbody>
	<tr class="ALERTTABLE">
		<th class="ALERTTABLE" colspan="8">
			...
			<?=_("Conditions based on total flow summary")?>:
		</th>
	</tr>
...
?>
	...
	<td style="width:2em">
<?php if ( $i > 0 ) {
   		print "<a href='#null' onclick='DeleteRow($i);' title='"._("Delete row")."' ><IMG SRC='icons/trash.png' ";
		print "name='delete_row_$i' id='delete_row_$i' border='0' align='right' alt='"._("trash")."' $controls_display_style></a>\n";
} ?>
	</td>
	<td style="width:2em">
<?php 
	...
	if ( $_i < $num_ConditionList ) { 
   		print "<a href='#null' onclick='EnableRow($_i);' title='"._("Add new row")."'><IMG SRC='icons/plus.png' ";
		print "name='add_row_$i' id='add_row_$i' border='0' align='right' alt='"._("plus icon")."' $controls_display_style></a>\n";
	} ?>
	</td>
</tr>
	...
	<tr class="ALERTTABLE">
		<th class="ALERTTABLE" colspan="6">
			<input type="radio" name="type" id="type1" value="1" 
				...
				<?php echo "$checked $disabled";?>> 
			<?=_("Conditions based on individual Top 1 statistics")?>:
		</th>
	</tr>
...
?>
	...
	<td>
<?php 
	print "<input type='hidden' name='visible_$i' id='visible_$i' title='"._("row state")."' ";
	print "value='" . $alert["visible_$i"] . "'>\n";
...
<?php
	if ( count($alert_condition_plugin) == 0 ) {
		print "	<option value='-1' >"._("No alert plugins available")."</option>\n";
	} else {
		$i = 0;
...
?>
	</select><?=_("x condition = true, and block next trigger for")?>
...
<?php $checked = $alert['action_type'] == 0 ? 'checked' : ''; ?>
			<input type="checkbox" name="action_0" id="action_0" value="0" onClick="SetAction(0);"
				<?php echo "$checked $disabled";?>>&nbsp;<?=_("No action")?>
		</td>
	</tr>
	<tr>
		<td>
<?php $checked = ($alert['action_type'] & 1) > 0  ? 'checked' : ''; ?>
			<input type="checkbox" name="action_1" id="action_1" value="1" onClick="SetAction(1);"
				<?php echo "$checked $disabled";?>>&nbsp;<?=_("Send alert email")?>
		</td>
		<td><?=_("To")?>:</td>
		<td>
...
<?php $checked = ($alert['action_type'] & 2) > 0  ? 'checked' : ''; ?>
			<input type="checkbox" name="action_2" id="action_2" value="2" onClick="SetAction(2);"
				<?php echo "$checked $disabled";?>>&nbsp;<?=_("Run system command")?>
		</td>
		<td colspan="2">
<?php $_disabled = $disabled || $checked == '' ? 'disabled' : '';
...
?>
			<input type="checkbox" name="action_3" id="action_3" value="3" onClick="SetAction(3);"
				<?php echo "$checked $disabled";?>>&nbsp;<?=_("Call plugin")?>:
		</td>
		<td colspan="2">
<?php
...
<tbody>
	...
	<tr>
		<td><?=_("Last cycle")?>: <?php echo $updated;?></td>
	</tr>
	<tr>
		<td class="MYVALUE">
<?php
...
<?php
		if ( array_key_exists('last_condition', $alert) ) {
			...
			$num = 0;
			print "<td><p><b>"._("Last conditions not available")."</b></td>\n";
		}
?>
	</tr>
	<tr>
...
<?php
		if ( $num ) {
			...
			print "<td><b>"._("Conditions").":</b></td>\n";
			for ( $i=0; $i< $num; $i++ ) {
				print "<td><b>";
				print $i+$offset;
				print "</b></td>\n";
			}
			print "<td><b>"._("Final").":</b></td>";
			print "</tr>\n";
			print "<tr>\n";
			print "<td><b>"._("State").":</b></td>\n";
			for ( $i=0; $i< $num; $i++ ) {
				print "<td>";
...
<tbody>
	<tr class="ALERTTABLE">
		<th class="ALERTTABLE" colspan="5">
			<input type="image" name="new_alert" value="new_alert" title="<?=_("Add new alert")?>"
...
<?php if ( count($alertstatus) == 0 ) { ?>
	<tr class="ALERTTABLE">
		<td class="MYVALUE"><b><?=_("No Alerts defined")?>.</b></td>
	</tr>
<?php } else { ?>
	<tr class="ALERTTABLE">
		<th class="ALERTTABLE"><?=_("No")?>.</th>
		<th class="ALERTTABLE"><?=_("Status")?></th>
		<th class="ALERTTABLE"><?=_("Name")?></th>
		<th colspan="2"class="ALERTTABLE"><?=_("Last Triggered")?></th>
	</tr>
<?php 
		$i = 0;
...
?>
	...
	<tr class='ALERTTABLE'>
		...
		<td class="MYVALUE" style="width:5em">
			<input type="image" name="view_alert" value="<?php echo $alertname;?>" title="<?=_("View alert")?>"
				src="icons/spyglas.png" onClick="SetAlertName('<?php echo $alertname;?>');" align="left">
			<input type="image" name="delete_alert" value="<?php echo $alertname;?>" title="<?=_("Delete alert")?>"
...
<?php if ( $new_alert ) { ?>
	<tr class="ALERTTABLE" colspan="2">
		<th class="ALERTTABLE" colspan="3"><?=_("New alert")?></th>
	</tr>
	<tr class="ALERTTABLE">
		<td class="ALERTLIST" style="width:5em"><?=_("Name")?></td>
		<td class="MYVALUE">
		<input type="text" name="alert" value="<?php  echo $alert['alert'];?>" size="24">
		</td>
	</tr>
	<tr class="ALERTTABLE">
		<td class="ALERTLIST" style="width:5em"><?=_("Status")?></td>
		<td class="MYVALUE">
			<input type="checkbox" name="status" id="status"  value="enabled" <?php echo $checked;?>><?=_("enabled")?>
		</td>
	</tr>
<?php } else {
...
?>
	<tr class="ALERTTABLE">
		<th class="ALERTTABLE" colspan="4"><?=_("Alerts details")?>: <?php echo $alert['name'];?></th>
	</tr>
	<tr class="ALERTTABLE">
		<th class="ALERTTABLE"><?=_("Trigger")?></th>
		<th class="ALERTTABLE"><?=_("Status")?></th>
		<th class="ALERTTABLE" colspan="2"><?=_("Last Triggered")?></th>
	</tr>
	<tr class='ALERTTABLE'>
		<td class="MYVALUE" style="width:6em;background-color:<?php echo $background;?>"><?php echo $label;?></td>
		<td class="MYVALUE">
			<input type="checkbox" name="status" id="status" onclick="this.form.submit();" 
			value="enabled" <?php echo $checked;?>><?=_("enabled")?>
...
?>
		...
		<td class="MYVALUE" style="width:5em">
			<input type="hidden" name="alert" id='alert' value="<?php  echo $alert['name'];?>" >
   			<a href="#null" onclick="" title="<?=_("Edit alert")?>" ><IMG SRC="icons/edit.png" onClick="EnableEdit(<?php echo "$cond_plugins, $action_plugins"; ?>);"
				name="edit_row" border="0" align="left" alt="<?=_("trash")?>"></a>
			<input type="image" name="delete_alert" value="" title="<?=_("Delete alert")?>" src="icons/trash.png"
...
?>
	<tr class='ALERTTABLE'>
    	<td class="ALERTTABLE" colspan="2">
			<input name='new_alert_cancel' value='<?=_("Cancel")?>' type='submit'>
			<input name='new_alert_commit' value='<?=_("Create Alert")?>' type='submit'>
		</td>
	</tr>
<?php
...
?>
		<input name="edit_alert_cancel" value="<?=_("Cancel")?>" type="submit" align='right'>
		<input name="edit_alert_commit" value="<?=_("Commit Changes")?>" type="submit" align='right'>
	</td></tr>
...
ob_start();
print _("Add/modify alert - alertinfo")."\n";
print_r($alertinfo);
...
ReportLog("Alert action eval: " . $_SESSION['action']);
	...
	switch ( $_SESSION['action'] ) {
		...
		case 'details':
			...
			break;
		default:
			print "<h3>"._("ERROR action").": " . $_SESSION['action'] . "</h3>";
